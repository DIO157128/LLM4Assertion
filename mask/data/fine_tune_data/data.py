...
def transmask(name,path1,path2):
    f1 = open(path1,'r',encoding='utf-8')
    f2 =  open(path2,'r',encoding='utf-8')
    source = f1.readlines()
    target = f2.readlines()
    ...
    for s,t in zip(source,target):
        test_prefix = s.split('"<FocalMethod>"')[0].strip()
        focal_method = s.split('"<FocalMethod>"')[1].strip()
        processed_s.append('"<FocalMethod>" ' + focal_method + '"<TestPrefix>" ' + test_prefix)
        processed_t.append(t)
    df = pd.DataFrame()
    df['source'] = processed_s
    df['target'] = processed_t
    df.to_csv("assert_mask_ori_new_{}.csv".format(name),index=False,escapechar="\\",encoding='utf-8')
if __name__ == '__main__':
    transmask("train","./NewDataSet/Training/testMethods.txt","./NewDataSet/Training/assertLines.txt")
    transmask("val", "./NewDataSet/Validation/testMethods.txt", "./NewDataSet/Validation/assertLines.txt")
    transmask("test", "./NewDataSet/Testing/testMethods.txt", "./NewDataSet/Testing/assertLines.txt")
